...
#include "../api_for_osxiec_script.h"
...
void execute_script_file(const char *filename) {
  ...
  if (len <= ext_len ||
      strcmp(filename + len - ext_len, SCRIPT_EXTENSION) != 0) {
...
void execute_cs_command(const char *command) {
  ...
  if (strcmp(cmd, "SET_MEMORY") == 0 && arg_count == 2) {
    ...
    scale_container_resources(soft_limit, hard_limit,
                              -1); // -1 for unchanged CPU priority
  } else if (strcmp(cmd, "SET_CPU") == 0 && arg_count == 1) {
    int cpu_priority = atoi(args[0]);
    scale_container_resources(-1, -1,
                              cpu_priority); // -1 for unchanged memory limits
    ...
    execute_command(full_command, NULL);
